...
for( pid in startNo:endNo ){
  if(nchar(pid) == 1) temp = "00"
    ...
    imgurl   <- xpathSApply(xmldoc, "//table[@class='tableBorder fill']//div[@class='divLineBlock']//a//img", xmlGetAttr, "title")
    ...
    EL = strtoi(EL)
      ...
      Erroresult<- tryCatch({
        subdata <- data.frame(urlPath, name, level, HP, MP, AD, AR, EL,imgurl)
...
print(nrow(alldata))
write.csv(alldata, file = "/Users/walter/Desktop/monster2.csv")